...
/**
 * @brief  Algoritmo Quick Union, faz a conexão entre 2 números p e q
 * @note   
...
void connectivityAdjacentQU(int *id, int *board, int tile, int n_Lines, int n_Col)
{
    ...
    if (checkInsideBoardP(aux, n_Lines, n_Col, 1) == 1) //1-soma horizontal
    {
        if (board[aux] == 0)
        {
            QuickUnion(id, tile, aux); //união entre tile e aux
        }
    }
...
/**
 * @brief  Verifica se 2 peças estão na mesma sala com base n o algoritmo de QuickUnion
...
int sameRoomQU(int *board, int n_Lines, int n_Col, int p1, int p2)
{
    ...
    for (i = p1; i < N; i++)
    {
        if (board[i] == 0)
        {
            ...
            connectivityAdjacentQU(id, board, i, n_Lines, n_Col); //verifica conetividade com as peças adjacentes
        }
    }
