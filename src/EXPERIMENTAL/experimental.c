...
void experimental(FILE *fp, boardRules *brp, bool *valido, bool *especifico, bool first, char *output)
{
    ...
    experimentalPrint(output, G, salaDoTesouro, valido, especifico, first, getBoardColumns(brp));
    if ( (*valido) && !(*especifico) )
        graphDestroy(G);
 ...
 * @retval
 */
void experimentalPrint(char *output, Graph *G, int salaDoTesouro, bool *valido, bool *especifico, bool first, int colunas)
{
    if ( (*valido) && !(*especifico) )
    {
        writeSolution(output, G, salaDoTesouro, colunas, first);
    }
    else
    {
        if (*especifico)
